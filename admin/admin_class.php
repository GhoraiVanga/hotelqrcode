...
function change_password(){
	extract($_POST);
$check = $this->db->query("SELECT  * FROM `admin` WHERE  `admin_password` = '$password1' ")->fetch_object()->admin_password;
if($check == $password1){
	return 2;
			exit;
		}
	...
	{
	   $save = $this->db->query("UPDATE `admin` SET `admin_password`='$password2'");
	   return 1;
	   exit;
	}
